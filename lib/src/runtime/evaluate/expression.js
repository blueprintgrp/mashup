...
function evaluateStringBinaryExpression(leftHandSide, rightHandSide, operator) {
    ...
    if (operator == '+')
        ...
        console.log("Cannot use operator \"".concat(operator, "\" in string binary expression."));
        process.exit(1);
    }
...
function evaluateAssignment(node, env) {
    if (node.assigne.kind !== 'Identifier') {
        console.log("Invalid left hand side in assignment expression ".concat(JSON.stringify(node.assigne), "."));
        process.exit(1);
    }
...
function evaluateMemberExpression(expression, env) {
    ...
    if (object.type === 'object') {
        ...
        if (propertyValue !== undefined) {
            ...
            console.log("Property \"".concat(property, "\" does not exist on object."));
            process.exit(1);
        }
    }
    else {
